...
// Persists per block via the /tasks API. The AI "refresh from folder" and the daily
// cron write into the same store (added in later phases).
...
export default function TasksView() {
  ...
  const inputRefs = useRef(new Map());        // id -> input element
  ...
  const onText = (id, text) => { patchLocal(id, { text }); saveText(id, text); };
  ...
  const setRef = (id) => (el) => { if (el) inputRefs.current.set(id, el); else inputRefs.current.delete(id); };
    ...
    <div style={{ flex: 1, height: '100%', overflow: 'auto', background: 'var(--bg-primary)' }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 28px 120px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
          ...
          <button onClick={() => addBlock('heading', blocks.length ? blocks[blocks.length - 1].id : undefined)}
        ...
        {loading ? (
          ...
          <div style={{ color: 'var(--text-tertiary)', fontSize: 14, lineHeight: 1.6 }}>
            ...
            {blocks.map((b, idx) => b.kind === 'heading' ? (
              ...
              <div key={b.id} className="task-row"
                ...
                <input
                  ...
                  style={{
                    flex: 1, border: 'none', outline: 'none', background: 'transparent',
                    fontSize: 14.5, lineHeight: 1.4, padding: '1px 0',
                    color: b.done ? 'var(--text-tertiary)' : 'var(--text-primary)',
                    textDecoration: b.done ? 'line-through' : 'none',
                  }}
                />
                {b.source === 'ai' && (
                  <span title={t('tasksView.fromEmail', { defaultValue: 'From email' })}
                    style={{ marginTop: 3, fontSize: 10, color: 'var(--accent)', flexShrink: 0 }}>✉</span>
                )}
                <button onClick={() => removeBlock(b, blocks[idx - 1]?.id)} style={rowDel} title="Remove">×</button>
              </div>
            ))}
